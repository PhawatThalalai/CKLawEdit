...
class DataCompromise extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->type == 'InsertComFin') {
            ...
            try {
                ...
                $Compro->totalSum -= (float)$request->data['pay_amount'];
                $Compro->SumInterest -= (float)$request->data['pay_amount'];
        ...
        if ($request->type == 'InsertComStopdate') {
            DB::beginTransaction();
            try {
                $Com_Stopdate = new Com_Stopdate();
                $Com_Stopdate->date = $request->data['stopdate'];
                $Com_Stopdate->cus_id = $request->data['cus_id'];


                $Com_Stopdate->note = $request->data['note'];
                $Com_Stopdate->save();
                DB::commit();
                ...
                // $renderHTML = view('DataCustomer.section-contract.view', compact('data', 'dataStatus', 'type', 'dataGuarantor', 'customer', 'finance', 'financeOther'))->render();
                return response()->json(['message' => $message, 'success' => '1', 'code' => 200]);
                // return  redirect()->route('Law.show', [$request->data['cus_id'], 'type' => 'showCus']);
            } catch (\Exception $e) {
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($request->type == 'NewCompro') {
            ...
            $stopdate = Com_Stopdate::where('cus_id', $id)->orderBy('id', 'ASC')->get();
            DB::beginTransaction();
            try {
                ...
                return  view('DataCompromise.section-NewCompromise.view', compact('data', 'dataGuarantor', 'customer', 'Compro', 'ComFin', 'ComFinTotal', 'CountExe', 'Tribunal', 'ComInstall', 'status_tag', 'Tracking', 'today', 'stopdate'));
        ...
        if ($request->type == 'InsertStopdate') {
            $cus_id =  $id;
            return view('DataCompromise.stopdate-com.insert-stopdate', compact('cus_id'));
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->type == 'updateCom') {
            $principalCal = 0;
            if ($request->data['type_interest'] == 'ครบสัญญา') {
                $principalCal =  (float)str_replace(",", "", $request->data['principal']) + ((($request->data['interest'] / 100 * (float)str_replace(",", "", $request->data['principal'])) / 12  * $request->data['installments']));
            } else {
                $principalCal = (float)str_replace(",", "", $request->data['principal']);
            }

            // dd($request->data['pay_com'],$request->data['period'],$request->data['installments']);
            if ($request->data['type_interest'] == 'ยอดคงเหลือ') {
                $final_in  = (float)str_replace(",", "", $request->data['pay_com']) - ((float)str_replace(",", "", $request->data['period']) * ($request->data['installments'] - 1));
            } else {
                $final_in  = $principalCal - ((float)str_replace(",", "", $request->data['period']) * ($request->data['installments'] - 1));
            }
        ...
        if ($request->type == 'CancelComFin') {
            ...
            try {
                ...
                $Compro = Compromise::where('cus_id', $request->data['cus_id'])->orderBy('id', 'DESC')->first();
                $Compro->totalSum += $ComFinance->pay_amount;
                $Compro->SumInterest += $ComFinance->pay_amount;
    ...
    // public function calInterest($id, $type, $com_id)
    {
        ...
        foreach ($Compro as $item) {
            ...
            if ($ComInstall != NULL && $ComInstallDate != NULL) {
                ...
                if ($item->type_interest != 'ครบสัญญา') {
